    ...
    class BufferFile(object):
        ...
        def read(self, n=-1):
            if self.closed:
                return b''
            if len(self.data) == 0:
                piece = self.buffer.get()
                if piece:
                    self.data += b''.join(piece)
                while True:
                    try:
                        piece = self.buffer.get(block=False)
                        if piece:
                            self.data += b''.join(piece)
                    except queue.Empty:
                        break
...
def main():
    ...
    with record_audio(RATE, CHUNK) as audio_file:
        ...
        results = audio_sample.streaming_recognize('en-US', interim_results=True)
